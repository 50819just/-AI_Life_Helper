# PART 1 — Home / Dashboard（合併首頁） / Module Library Flow

## 文件用途
把 NestBuddy 的入口層、首頁、Ask AI、Settings 與 Module Library 拆成第一段流程，避免和三個正式模組內部細節混在一起。
...
## NESTBUDDY — Entry & Core Navigation
...
│   │
│   ├── Ask AI Command Bar
│   │   └── AI Ask anything / 建議按鈕 / 類別 chips
│   │
│   ├── Dashboard Summary（Home 內摘要區） / 快速狀態
│   │   ├── 環境 / 濕度
│   │   ├── 睡眠分析
│   │   └── 快速動作
│   │
...
- Dashboard Summary（Home 內摘要區） 是 Home 內的狀態摘要區塊，不預設成獨立主導航頁。
- 現行 sidebar 的 `Dashboard` 代表首頁入口，避免做出兩套衝突命名。
